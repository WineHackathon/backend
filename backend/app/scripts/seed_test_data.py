...
async def seed_test_data():
    ...
    try:
        ...
        if not wines:
            logger.error("В каталоге нет вин! Сначала загрузите каталог.")
            return
        ...
        if not user:
            ...
            session.add(user)
            await session.flush()
            logger.info(f"Создан тестовый пользователь: {test_email} / password123")
        else:
            user.password_hash = hash_password("password123")
            await session.flush()
            logger.info(f"Обновлен пароль тестового пользователя: {test_email} / password123")
        ...
        if not admin:
            ...
            session.add(admin)
            await session.flush()
            logger.info(f"Создан администратор: {admin_email} / adminpassword123")
        else:
        ...
        for idx, wine in enumerate(wines[:5]):
            status = CellarStatus.IN_CELLAR if idx < 2 else (CellarStatus.WISHLIST if idx < 4 else CellarStatus.TASTED)
            ...
            if not existing_cellar:
                cellar_item = UserCellar(
                    ...
                    tasting_notes=f"Отличный образец российского виноделия ({wine.name}). Прекрасный баланс и долгое послевкусие.",
                )
                session.add(cellar_item)

        # 5. Создаем историю сканирований пользователя
        ...
        if not guest_scans_exist and len(wines) >= 2:
            ...
            logger.info(f"Созданы гостевые сканы для фингерпринта {demo_fp}")

        # 7. Создаем историю предпочтений сомелье
        ...
        logger.info("Тестовые данные успешно записаны в базу данных!")
    finally:
        await session.close()
